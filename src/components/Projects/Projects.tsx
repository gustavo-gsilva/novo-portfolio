   ...
   return (
      <section className="flex flex-col items-center xl:items-stretch xl:flex-row justify-center gap-20 my-5 mx-10 ">
         <div className="xl:hidden"> 
            <img className={`
               sm:max-w-[620px] xl:w-auto object-cover mt-20 rounded-3xl
         ...
         <div>
            <h4 className="text-blue-50 text-[7.5rem] font-semibold">{project.number}</h4>

            <h3 className="text-blue-50 text-[4rem] font-medium mt-5">{project.title}</h3>

            <p className="max-w-5xl text-blue-50 text-[1.8rem] mt-11">{project.description}</p>
            ...
            <div className="flex flex-wrap gap-8 border-b-2 border-blue-50 pb-11 mt-11">
               {project.techs.map((tech) => (
                  <div className="flex flex-col sm:flex-row items-center gap-5" key={tech.name}>
                     ...
                     <p className="text-blue-50 text-[1.8rem] font-medium">{tech.name}</p>
                  </div>
               ))}
            </div>
         ...
         <div className="hidden xl:block"> {/* Container versão desktop */}
            <img className={`
               max-w-[580px] object-cover mt-5 rounded-3xl
               transition-all duration-300
               ${animating && slideDirection === "right" ? "translate-x-32 opacity-0" : ""}
               ${animating && slideDirection === "left" ? "-translate-x-32 opacity-0" : ""}
               ${!animating ? "translate-x-0 opacity-100" : ""}
